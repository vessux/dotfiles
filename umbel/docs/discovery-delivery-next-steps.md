...
4. **Migrate the borklog backlog.** `docs/backlog.jsonl` (gitignored; 8 items — 6 multi-harness adapters, 1 shim-publish `bk-2026-05-30T14:50:52Z`, 1 YAML-parse papercut `bk-2026-06-01T09:59:33Z`) → `bd q` into the inbox → triage → flesh kept ones into **GitHub Issues** and close the bead; drop the rest.
