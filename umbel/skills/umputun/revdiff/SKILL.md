...
- "revdiff last", "review my last reply", "annotate previous response", "revdiff this conversation", "review what you just said" — review the previous completed assistant turn (latest-conversation mode)
...
## Reviewing Latest Conversation

When the user says "revdiff last" or similar (see Activation Triggers), run:

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/latest-conversation.sh
```

The script extracts the previous completed assistant turn from the Claude Code
session transcript (the assistant text bounded by the last two typed-user
prompts, so the in-flight turn is never included), writes it to a temp
markdown file, opens revdiff in the standard overlay launcher with
`--only=<temp>`, and post-processes returned annotations to inline a fenced
quote of the referenced source lines under each `## file:line (type)` header.

Skip Step 1 (ref detection) and Step 2 (invocation) — this script wraps both.
Treat the returned annotations as user feedback on your previous reply and
continue from there. Empty output → no completed assistant turn yet; mention
